...
const MeetingBookingPage = () => {
  ...
  useEffect(() => {
    // Get meeting data from location state or fetch it
    if (location.state?.meetingData) {
      setMeeting(location.state.meetingData);
      setLoading(false);
      
      // Auto-select first available date
      if (location.state.meetingData.availableSlots && location.state.meetingData.availableSlots.length > 0) {
        const firstAvailableDate = location.state.meetingData.availableSlots.find(slot => slot.available)?.date;
        if (firstAvailableDate) {
          setSelectedDate(firstAvailableDate);
        }
      }
    } else {
      // If no state data, simulate fetching meeting data
      fetchMeeting();
    }
  ...
  const fetchMeeting = async () => {
    try {
      ...
      // Mock meeting data - replace this with your actual data source
      const mockMeeting = {
        id: meetingId,
        title: 'Project Review Meeting',
        description: 'Weekly project review and planning session for the Q1 initiatives.',
        duration: 60,
        meetingType: 'online',
        ...
        availableSlots: [
          ...
          { key: '1', date: new Date().toISOString().split('T')[0], time: '10:00', available: true },
          { key: '2', date: new Date().toISOString().split('T')[0], time: '14:00', available: true },
          { key: '3', date: new Date().toISOString().split('T')[0], time: '16:00', available: true },
          
          // Tomorrow
          { key: '4', date: new Date(Date.now() + 24*60*60*1000).toISOString().split('T')[0], time: '09:00', available: true },
          { key: '5', date: new Date(Date.now() + 24*60*60*1000).toISOString().split('T')[0], time: '13:00', available: true },
          { key: '6', date: new Date(Date.now() + 24*60*60*1000).toISOString().split('T')[0], time: '15:00', available: true },
          
          // Day after tomorrow
          { key: '7', date: new Date(Date.now() + 2*24*60*60*1000).toISOString().split('T')[0], time: '10:00', available: true },
          { key: '8', date: new Date(Date.now() + 2*24*60*60*1000).toISOString().split('T')[0], time: '14:00', available: true },
          
          // 3 days from now
          { key: '9', date: new Date(Date.now() + 3*24*60*60*1000).toISOString().split('T')[0], time: '11:00', available: true },
          { key: '10', date: new Date(Date.now() + 3*24*60*60*1000).toISOString().split('T')[0], time: '16:00', available: true },
          
          // Some booked slots for testing
          { key: '11', date: new Date(Date.now() + 4*24*60*60*1000).toISOString().split('T')[0], time: '10:00', available: false, bookedBy: 'john@example.com' },
          { key: '12', date: new Date(Date.now() + 4*24*60*60*1000).toISOString().split('T')[0], time: '14:00', available: false, bookedBy: 'sarah@example.com' },
        ]
      };
      ...
      // Auto-select first available date
      if (mockMeeting.availableSlots && mockMeeting.availableSlots.length > 0) {
        const firstAvailableDate = mockMeeting.availableSlots.find(slot => slot.available)?.date;
        if (firstAvailableDate) {
          setSelectedDate(firstAvailableDate);
        }
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updatedSlots = meeting.availableSlots.map(slot => {
  ...
  const generateCalendarDays = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const hasSlots = meeting?.availableSlots?.some(slot => 
  ...
  const getAvailableSlotsForDate = (date) => {
    if (!meeting?.availableSlots) return [];
    return meeting.availableSlots.filter(slot => 
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500">Please check the link or contact the meeting organizer.</p>
        </div>
      </div>
    );
  }
          ...
          <div className="lg:col-span-1">
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6 h-fit hover:shadow-2xl transition-all duration-300">
              <div className="flex items-center mb-4">
                ...
                <h2 className="text-lg font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">Meeting Details</h2>
              </div>
          ...
          <div className="lg:col-span-3">
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6 hover:shadow-2xl transition-all duration-300">
              <div className="text-center mb-4">
                <h2 className="text-3xl font-bold bg-gradient-to-r from-gray-800 via-blue-700 to-indigo-700 bg-clip-text text-transparent mb-2">
                    ...
                    {day.isEmpty ? (
                      ...
                      >
                        <div className="relative flex items-center justify-center h-full">
                          ...
                          {day.hasSlots && (
                            <div className="absolute -top-1 -right-1 w-2 h-2 bg-gradient-to-r from-blue-400 to-indigo-500 rounded-full shadow-sm"></div>
                          )}
                        </div>
                      </button>
                    )}
            ...
            {showBookingForm && selectedSlot && formData && (
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-5 hover:shadow-2xl transition-all duration-300">
                ...
                <form onSubmit={handleSubmit} className="space-y-4" noValidate>
                  ...
                  >
                    {submitting ? 'Booking Meeting...' : 'Book Meeting'}
                  </button>
                </form>
              </div>
            )}
